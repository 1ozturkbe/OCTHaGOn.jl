...
function train_transonic_tree()
    ...
    vks = ["Re", "thick", "M", "C_L"];
    X = CSV.read("../../data/airfoil/airfoil_X.csv", copycols=true, header=vks, delim=",");
...
function transonic_mio_model()
    ...
    lnr = IAI.read_json("data/transonic_tree.json");
    ...
    m = add_regr_constraints(m, x, y, lnr, vks, M=100, eq=true);
