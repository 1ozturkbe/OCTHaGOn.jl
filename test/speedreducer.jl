...
function speed_reducer()
    ...
    # Objective
    add_constraint(gm, @NLexpression(gm.model, x[8] - (0.7854*x[1]*x[2]^2*(10. / 3. *x[3]^2 + 14.9334*x[3] - 43.0934) -
             1.508*x[1]*(x[6]^2 + x[7]^2) + 7.477*(x[6]^3 + x[7]^3) +
             0.7854*(x[4]*x[6]^2 + x[5]*x[7]^2))), vars = all_vars)


    # Constraints
    add_constraint(gm, @NLexpression(gm.model, g1, -27 + x[1] * x[2]^2 * x[3]), vars = x[1:3])

    add_constraint(gm, @NLexpression(gm.model, g2, -397.5 + x[1] * x[2]^2 * x[3]^2), vars = x[1:3])

    add_constraint(gm, @NLexpression(gm.model, g3, -1.93 + x[2] * x[6]^4 * x[3] / x[4]^3),  vars = [x[2], x[3], x[4], x[6]])

    add_constraint(gm, @NLexpression(gm.model, g4, -1.93 + x[2] * x[7]^4 * x[3] / x[5]^3), vars = [x[2], x[3], x[5], x[7]]))

    add_constraint(gm, @NLexpression(gm.model, g5, 1100 - (((745*x[4]/(x[2]*x[3]))^2) +
                                            16.91*10^6)^0.5/(0.1*x[6]^3)), vars = [x[2], x[3], x[4], x[6]])

    add_constraint(gm, @NLexpression(gm.model, g6, 850 - (((745*x[5]/(x[2]*x[3]))^2) +
                                            157.5*10^6)^0.5/(0.1*x[7]^3)), vars = [x[2], x[3], x[5], x[7]])

    add_constraint(gm, @NLexpression(gm.model, g7, 40 - x[2]*x[3]), vars = [x[2], x[3]])

    add_constraint(gm, @NLexpression(gm.model, g8, x[1]/x[2] - 5), vars = [x[1], x[2]])

    add_constraint(gm, @NLexpression(gm.model, g9, 12 - x[1]/x[2]), vars = [x[1], x[2]])

    add_constraint(gm, @NLexpression(gm.model, g10, x[4] - 1.5*x[6] - 1.9), vars = [x[4], x[6]])
